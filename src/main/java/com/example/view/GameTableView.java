...
public class GameTableView extends LayeredPane {
    ...
    private static final double CENTER_ATTRACTION_STRENGTH = 0.1;

    private static GameTableView instance;

    private Map<Tecton, Point> tectonPositions;
    private GameTable gameTable;
    private TectonView selectedTecton = null;

    private GameTableView(GameTable gameTableIn) {
        gameTable = gameTableIn;
        ...
        tectonPositions = initializePositions(gameTable.getTectons());
    ...
    public static GameTableView getInstance(GameTable gameTable) {
        if (instance == null) {
            if (gameTable == null) {
                throw new IllegalArgumentException("GameTable must not be null for the first getInstance call");
            }
            instance = new GameTableView(gameTable);
        }
        return instance;
    }

    public void removeTectonPosition(Tecton tecton) {
        tectonPositions.remove(tecton);
    }

    public void addTectonPosition(Tecton tecton) {
...
//        }
        ...
        // Adjusted constants
        final double coolingRate = 0.99; // Cooling factor per iteration
        ...
        for (int iter = 0; iter < MAX_ITERATIONS; iter++) {
            ...
            for (Tecton tecton : tectons) {
                ...
                dx += CENTER_ATTRACTION_STRENGTH * (centerX - currentPos.x);
                dy += CENTER_ATTRACTION_STRENGTH * (centerY - currentPos.y);
